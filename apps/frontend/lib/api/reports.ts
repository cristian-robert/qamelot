...
export const reportsApi = {
  ...
  getActivity: (projectId: string, dateRange?: DateRangeFilter) =>
    apiFetch<ActivityReportDto>(
      `/projects/${projectId}/reports/activity${buildDateParams(dateRange)}`,
    ),
  ...
  getComparison: (projectId: string, runIdA: string, runIdB: string) =>
    apiFetch<ComparisonReportDto>(
      `/projects/${projectId}/reports/comparison?runIdA=${encodeURIComponent(runIdA)}&runIdB=${encodeURIComponent(runIdB)}`,
    ),

  getDefectSummary: (projectId: string, dateRange?: DateRangeFilter) =>
    apiFetch<DefectSummaryReportDto>(
      `/projects/${projectId}/reports/defect-summary${buildDateParams(dateRange)}`,
    ),

  getUserWorkload: (projectId: string, dateRange?: DateRangeFilter) =>
    apiFetch<UserWorkloadReportDto>(
      `/projects/${projectId}/reports/user-workload${buildDateParams(dateRange)}`,
    ),
